...
// Tests para DfsSolver.
// Tienen la misma estructura que BfsSolverTests para que sea facil compararlos.
// La diferencia mas grande es que DFS no promete darte el camino mas corto,
// solo te garantiza que si hay una salida, la va a encontrar.
[TestClass]
public class DfsSolverTests
{
    private readonly DfsSolver _solver = new();
    ...
    [TestMethod]
    [Description("En un pasillo recto de 3 celdas, DFS encuentra el camino correcto paso a paso.")]
    ...
    // laberinto en L: solo existe una ruta, DFS tiene que encontrarla si o si

    [TestMethod]
    [Description("Cuando hay un unico camino en forma de L, DFS lo encuentra sin problemas.")]
    ...
    // con dos caminos posibles, DFS no te asegura cual va a tomar,
    // solo nos importa que llegue a la meta y el camino tenga sentido

    [TestMethod]
    [Description("Con dos rutas posibles, DFS agarra cualquiera que llegue a la meta (no promete la mas corta).")]
    public void Solve_DosCaminos_DebeEncontrar_UnCaminoValido()
    {
        ...
        // DFS solo garantiza encontrar ALGUN camino, no el mas corto
        Assert.IsTrue(result.PathFound);
        Assert.IsTrue(result.Path.Count >= 5, "El camino minimo tiene 5 celdas.");
        Assert.AreEqual(start.Row, result.Path[0].Row);
        Assert.AreEqual(start.Col, result.Path[0].Col);
        Assert.AreEqual(goal.Row,  result.Path[^1].Row);
        Assert.AreEqual(goal.Col,  result.Path[^1].Col);
    }

    // comprobaciones rapidas de las metricas

    [TestMethod]
    [Description("AlgorithmName debe ser DFS.")]
    public void Solve_AlgorithmName_DebeSerDFS()
    {
        var maze   = MazeTestHelper.CreateLinear1x3();
        var result = _solver.Solve(maze, maze[0, 0], maze[0, 2]);

        Assert.AreEqual("DFS", result.AlgorithmName);
    }
